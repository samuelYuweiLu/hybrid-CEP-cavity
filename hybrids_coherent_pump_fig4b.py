...
ga = 0.0072  # coupling strength
gc = 0.000144  # coupling strength
g1 = -0.0029  # coupling strength
kr = 0.00245
kn = 0.2
kappa_1 = kn + kr       # cavity dissipation rate
kappa_c = 1.2/10000       # cavity dissipation rate
gamma_z = 0
omega = 0.1*gamma_s
dw0 = 0
...
def calulcate_avg_photons(dw):
    # Hamiltonian
    H_e = ( dw + dw0 ) * c1.dag() * c1 + ( dw + dw0 ) * c2.dag() * c2 + dw * a.dag() * a + dw * sm.dag() * sm + omega * (sm.dag() + sm) + \
    ...
    L3 = kappa_c * ( spre(c1) * spost(c1.dag()) - 0.5 * spost(c1.dag() * c1) - 0.5 * spre(c1.dag() * c1) )
    L4 = kappa_c * ( spre(c2) * spost(c2.dag()) - 0.5 * spost(c2.dag() * c2) - 0.5 * spre(c2.dag() * c2) )
...
wl_vec = np.linspace(-4*kappa_c, 4*kappa_c, 301)

for dw in wl_vec:
    nqe, nca, nc1, nc2, n_pe = calulcate_avg_photons(dw)
    nqe_vec.append(nqe)
    nca_vec.append(nca)
    nc1_vec.append(nc1)
    nc2_vec.append(nc2)
...
axes.plot(-wl_vec/kappa_c, np.log10(nca_vec), label="Plasmon")
axes.plot(-wl_vec/kappa_c, np.log10(nc1_vec), label="CCW")
axes.plot(-wl_vec/kappa_c, np.log10(nc2_vec), label="CW")
axes.plot(-wl_vec/kappa_c, np.log10(nqe_vec), label="QE")
...
fig, axes = plt.subplots(1, 1, figsize=(6,4))
axes.plot(-wl_vec/kappa_c, eta_vec, label="Quantum yield")
axes.set_xlabel(r'$\Delta\omega_l / \kappa_c$', fontsize=18);
axes.set_ylabel(r'$n_{ss}$', fontsize=18);
# axes.legend(loc=0)
